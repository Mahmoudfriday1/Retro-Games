    ...
    <div className="min-h-screen bg-[#050505] text-white w-screen m-0 p-0 overflow-x-hidden relative">
      ...
      <nav className="sticky top-0 left-0 right-0 z-[100] w-full bg-[#0a0a0a]/95 backdrop-blur-md border-b border-purple-600/30 py-4 flex justify-between items-center px-4 md:px-8">
        ...
        <div className="hidden lg:flex items-center gap-x-8 font-bold uppercase text-[12px] tracking-widest">
          ...
          <button onClick={handleLogout} className="text-red-500 hover:text-white transition uppercase text-[10px] font-black border border-red-500/20 px-3 py-1 rounded-full">Logout</button>
        </div>
            ...
            {allGames.map(game => (
              <div key={game.id} onClick={() => { setSelectedProduct(game); setView('details'); }} className="bg-[#111] rounded-lg overflow-hidden border border-white/5 hover:border-purple-600/50 transition-all group flex flex-col cursor-pointer">
                ...
                <div className="p-3 flex-grow flex flex-col justify-between">
                  <h3 className="font-bold text-[12px] md:text-[14px] mb-1 truncate uppercase text-gray-200">{game.name}</h3>
      ...
      {view === 'details' && selectedProduct && (
        <div className="w-full min-h-[90vh] py-10 px-4 md:px-20 flex flex-col lg:flex-row gap-12 items-center lg:items-start">
          <button onClick={() => setView('products')} className="absolute top-24 left-4 text-gray-400 hover:text-purple-500 font-bold uppercase text-[10px]">← Back to Store</button>
